...
eto_vremenno = {
    '(1)': 'За исключением входящих в указанные коды ОКПД2 товаров, работ, услуг, в случае осуществления закупок которых заказчик вправе проводить конкурс с ограниченным участием и двухэтап>
    '(2)': 'За исключением пищевых продуктов, закупаемых для дошкольных образовательных учреждений, общеобразовательных учреждений, образовательных учреждений начального профессионального, >
    '(3)': 'За исключением детской одежды.',
    '(4)': 'За исключением работ по строительству, реконструкции, капитальному ремонту особо опасных, технически сложных объектов капитального строительства, а также искусственных дорожных >
...
def create_zapret(request: Request, db: Session = Depends(get_db)):
    ...
    for el in codes:
        db_ogr = models.Prohibitions(numbers=el,
                                     name="Постановление Правительства РФ от 30 апреля 2020 г. N 616 Об установлении запрета на допуск промышленных товаров, происходящих из иностранных госу>
                                     exceptions=None)
        db.add(db_ogr)
        db.commit()
        db.refresh(db_ogr)
